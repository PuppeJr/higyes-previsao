...
function App() {
  ...
  // Histórico de pedidos (carregado do localStorage)
  ...
  // Calcular pedidos urgentes usando useMemo
  ...
  return (
    <div style={{ padding: '15px', maxWidth: '900px', margin: '0 auto', fontFamily: 'Arial', position: 'relative' }}>
      ...
      {mostrarAviso && (
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => setMostrarAviso(false)} style={{ float: 'right', background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer' }}>✕</button>
            ...
            {pedidosUrgentes.length === 0 ? (
              ...
              <>
                ...
                {pedidosUrgentes.map((p, i) => (
                  <div key={i} style={{ border: '1px solid #f44336', borderRadius: '8px', padding: '12px', marginBottom: '10px', backgroundColor: '#ffebee' }}>
      ...
      <h2 style={{ color: '#1a237e', textAlign: 'center' }}>🧼 Previsão de Pedidos - Higyés</h2>
      <p style={{ textAlign: 'center', color: '#666' }}>Monte o pedido e calcule quando o cliente deve comprar novamente</p>

      {/* Dados do Cliente */}
      <div style={{ backgroundColor: '#f5f5f5', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
        <h4 style={{ marginTop: 0 }}>📋 Dados do Cliente</h4>
        <div className="grid-3">
          <div>
            <label><strong>Cliente *</strong></label>
            <input type="text" value={clienteAtual} onChange={(e) => setClienteAtual(e.target.value)} placeholder="Nome" style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label><strong>Data do último pedido *</strong></label>
            <input type="date" value={dataUltimoPedido} onChange={(e) => setDataUltimoPedido(e.target.value)} style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label><strong>Consumo médio diário (caixas/dia)</strong></label>
            <input type="number" value={consumoDiario} onChange={(e) => setConsumoDiario(Number(e.target.value))} step="0.1" min="0.1" style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', boxSizing: 'border-box' }} />
          </div>
        </div>
        <div style={{ marginTop: '10px' }}>
          <label><strong>Observações (opcional)</strong></label>
          <input type="text" value={observacao} onChange={(e) => setObservacao(e.target.value)} placeholder="Ex: pedido grande, promoção..." style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', boxSizing: 'border-box' }} />
        </div>
      </div>
      ...
      <div style={{ backgroundColor: '#fff', padding: '15px', borderRadius: '8px', border: '1px solid #ddd', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '10px', alignItems: 'flex-end' }}>
          <div style={{ flex: '2 1 150px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#333' }}>Produto</label>
            <select 
          ...
          <div style={{ flex: '0 1 80px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#333' }}>Qtd</label>
            <input 
              type="number" 
          ...
          <div style={{ flex: '0 1 120px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#333' }}>Preço unit.</label>
            <input 
              type="number" 
              value={precoItem} 
        ...
        {itensPedido.length === 0 ? (
          <p style={{ color: '#999' }}>Nenhum item adicionado.</p>
        ) : (
      ...
      {previsao && (
        <div style={{ marginTop: '20px', padding: '20px', backgroundColor: 'white', borderRadius: '8px', border: `3px solid ${previsao.cor}`, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          <h3 style={{ margin: '0 0 15px 0', color: '#1a237e' }}>📈 Previsão para {previsao.cliente}</h3>
          <div style={{ backgroundColor: previsao.cor + '20', padding: '15px', borderRadius: '8px', marginBottom: '15px' }}>
            ...
            <p style={{ margin: '5px 0 0 0', fontSize: '14px', color: '#666' }}>{previsao.statusDesc}</p>
          </div>
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        {mostrarHistorico && historicoPedidos.length > 0 && (
          <div className="table-wrap" style={{ maxHeight: '400px', overflowY: 'auto', marginTop: '10px', backgroundColor: 'white', borderRadius: '8px', padding: '10px' }}>
            {historicoPedidos.map((pedido, index) => (
              <div key={pedido.id} onClick={() => carregarPedido(pedido)} style={{ cursor: 'pointer', padding: '12px', margin: '5px 0', backgroundColor: index % 2 === 0 ? '#f5f5f5' : '#e8eaf6', borderRadius: '4px', borderLeft: `4px solid ${pedido.cor}` }}>
                ...
                <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>Próxima compra: {pedido.dataProximaCompra} | Criado em: {pedido.dataCriacao}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ marginTop: '20px', padding: '10px', textAlign: 'center', fontSize: '12px', color: '#666' }}>
